...
fn run_test(e: Element, expected_result: [u16; 8], expected_acc_top: [u16; 8], expected_acc_mid: [u16; 8], expected_acc_low: [u16; 8]) -> Result<(), String> {
    // Prepare input data
    SPMEM::write_vector_16(0x00, &[0x0000, 0x8000, 0xFFFF, 0x8000, 0x8001, 0x8000, 0x7FFF, 0x8000]);
    ...
    soft_assert_eq(SPMEM::read_vector_16(0x100), expected_result, "VMADN result")?;
    soft_assert_eq(SPMEM::read_vector_16(0x110), expected_acc_top, "VMADN Acc[32..48]")?;
    soft_assert_eq(SPMEM::read_vector_16(0x120), expected_acc_mid, "VMADN Acc[16..32]")?;
    soft_assert_eq(SPMEM::read_vector_16(0x130), expected_acc_low, "VMADN Acc[0..8]")?;
...
impl Test for VMADNAccumulatorOverflowed {
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        // Prepare input data
        SPMEM::write_vector_16(0x00, &[0x8321, 0x7FEC, 0, 0, 0, 0, 0, 0]);
        SPMEM::write_vector_16(0x10, &[0x8123, 0x8123, 0, 0, 0, 0, 0, 0]);
        ...
        soft_assert_eq(SPMEM::read_vector_16(0x100), [0, 0, 0x8000, 0x8000, 0x8000, 0x8000, 0x8000, 0x8000], "Result after accumulator overflow in [1]")?;
        soft_assert_eq(SPMEM::read_vector_16(0x110), [0x830d, 0x8000, 0, 0, 0, 0, 0, 0], "Acc[32..48] after accumulator overflow in [1]")?;
        soft_assert_eq(SPMEM::read_vector_16(0x120), [0x5e14, 0x1cb6, 0, 0, 0, 0, 0, 0], "Acc[16..32] after accumulator overflow in [1]")?;
        soft_assert_eq(SPMEM::read_vector_16(0x130), [0xd876, 0x85c8, 0x8000, 0x8000, 0x8000, 0x8000, 0x8000, 0x8000], "Acc[0..8] after accumulator overflow in [1]")?;

        // After second overflow
        soft_assert_eq(SPMEM::read_vector_16(0x140), [0xffff, 0, 0x8000, 0x8000, 0x8000, 0x8000, 0x8000, 0x8000], "Result after accumulator overflow in [0]")?;
        soft_assert_eq(SPMEM::read_vector_16(0x150), [0x7fff, 0x8320, 0, 0, 0, 0, 0, 0], "Acc[32..48] after accumulator overflow in [0]")?;
        soft_assert_eq(SPMEM::read_vector_16(0x160), [0xe5cc, 0xaca8, 0, 0, 0, 0, 0, 0], "Acc[16..32] after accumulator overflow in [0]")?;
        soft_assert_eq(SPMEM::read_vector_16(0x170), [0xe1ae, 0x7968, 0x8000, 0x8000, 0x8000, 0x8000, 0x8000, 0x8000], "Acc[0..8] after accumulator overflow in [0]")?;
