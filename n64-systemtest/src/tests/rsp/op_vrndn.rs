...
fn run_test(e: Element, vs: VR, vt: VR, expected_result: [u16; 8], expected_acc_top: [u16; 8], expected_acc_mid: [u16; 8], expected_acc_low: [u16; 8]) -> Result<(), String> {
    ...
    assembler.write_vrndn(VR::V2, vt, vs, e);
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        for i in (1..32).step_by(2) {
            ...
            run_test(
                ...
                VR::from_index(i).unwrap(),
                vt,
                [0, 1, 1, 0, 1, 0x7fff, 0x7fff, 0x8000],
                [0, 0, 0, 0, 0, 0x3fff, 0x1fff, 0xc000],
                [0, 1, 1, 0, 1, 1, 0x4001, 2],
...
impl Test for VRNDNAccumulatorOverflowed {
    ...
    fn run(&self, _value: &Box<dyn Any>) -> Result<(), String> {
        // Prepare input data
        SPMEM::write_vector16_into_dmem(0x00, &[0x0001, 0x7FFF, 0, 0, 0, 0, 0, 0]);
        ...
        // Loop begin. After 32768 iterations the first and second element in the accumulator will flip from 0xFFFF to 0x0000
        assembler.write_li(GPR::A0, 32768);
